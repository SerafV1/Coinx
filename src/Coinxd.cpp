...
bool AppInit(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Coinx Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";

        if (mapArgs.count("-version")) {
            strUsage += LicenseInfo();
        } else {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  Coinxd [options]                     " + _("Start Coinx Core Daemon") + "\n";
    ...
    try {
        ...
        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in pivxd anymore. Use the Coinx-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
        fDaemon = GetBoolArg("-daemon", false);
        if (fDaemon) {
            fprintf(stdout, "CoinX server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect Coinxd signal handlers
    noui_connect();
